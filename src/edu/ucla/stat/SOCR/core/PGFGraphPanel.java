...
public class PGFGraphPanel extends GraphPanels implements MouseListener,
MouseMotionListener {
	 ...
	 */
	public PGFGraphPanel(SOCRDistributionFunctors container) {
		super(container);
	}
	 ...
	 */
	public final static String PANELFILE = "implementedPGFs.txt";
	...
	public void setDistribution(Distribution d) {
		if (d != null) {
			dist = d;
			domain = dist.getPGFDomain(); // ** Changed by Rahul Gidwani to call getMGFDomain()
			type = CONTINUOUS;
			left = domain.getLowerBound();
			right = domain.getUpperBound();
			double y_max = dist.getPGF(right);
	...
	public void paintComponent(Graphics g) 
	{
		...
		if (dist != null) {
			...
			for (x = a; x < b; x = x + delta) {
				drawLine(g, x, dist.getPGF(x), x + delta, dist.getPGF(x
						+ delta));
			}
			a = left;
			b = right;
			for (x = a; x < b; x = x + delta) {
				drawLine(g, x, dist.getPGF(x), x, 0);
			}
			...
			if (xPosition > leftMargin && xPosition < getWidth() - rightMargin
				...
				y1 = yGraph(dist.getPGF(xScale(xPosition)));
				...
				+ format(dist.getPGF(xScale(xPosition))) + " )";
	...
	public double getLeftCDF() {
		double left_mgf = dist.getPGF(left);
		return left_mgf;
	}

	public double getRightCDF() {
		double right_mgf = dist.getPGF(right);
		return right_mgf;
	}

...
public class GraphPanels extends JPanel implements MouseListener,
        MouseMotionListener {
    ...
    public GraphPanels(SOCRDistributionFunctors container) {
	...
	// return the filename for which distributions belong to this panel.
	...
	{	if (_left<=right && _left >=domain.getLowerBound()) {
		...
		else if (type == DISCRETE) this.container.leftCutOff.setText((new Double(left+0.5)).toString());
		else if (type == CONTINUOUS) this.container.leftCutOff.setText((new Double(left)).toString());
    }
	...
	{	if (_right>=left && _right<=domain.getUpperBound()) {
		...
		else if (type == DISCRETE) this.container.rightCutOff.setText((new Double(right-0.5)).toString());
		else if (type == CONTINUOUS) this.container.rightCutOff.setText((new Double(right)).toString());
	}
        ...
        if (x >= leftMargin && x < getWidth() - rightMargin) {
 	    ...
 	    ////System.out.println("GraphPanel mouseDragged xPosition = " + xPosition + ", yPosition = " + yPosition);
       	if (type == DISCRETE) {
        		...
        		this.container.leftCutOff.setText((new Double((int)(getLeftCutOff()+1.5))).toString());

	...
	*/
	public void paintComponent(Graphics g){
		...
		if (tossed){
			...
			g.setColor(textColor);
 			g.setFont(font);
			int x = g.getFontMetrics(font).stringWidth(labels[(int)value]);
			int y = g.getFontMetrics(font).getAscent();
			g.drawString(labels[(int)value], size / 2 - x / 2, size / 2 + y / 2);
		}
	}
	...
	public void paintCoin(Graphics2D g2, int x, int y ){
		...
		if(twoSides){
			...
			else if (tossed & value == 2) g2.setColor(Color.yellow);
		}
		...
		if (tossed){
			...
			int xx;
			if(twoSides)
				xx= g2.getFontMetrics(font).stringWidth(labels[(int)value]);
			else 
				xx =g2.getFontMetrics(font).stringWidth(Double.toString(value));
			int yy = g2.getFontMetrics(font).getAscent();
			if(twoSides)
					g2.drawString(labels[(int)value], size / 2 - xx / 2, size / 2 + yy / 2);
			else
				g2.drawString(Double.toString(value), size / 2 - xx / 2, size / 2 + yy / 2);
		}
	}

...
****************************************************/
package edu.ucla.stat.SOCR.analyses.model;   // line 229 changed
...
public class LogisticRegression implements Analysis {
	...
	private LogisticRegressionResult regression(ArrayList<Object> x, double[] y) 
		...
		double[] sortedResiduals = 
			(double[])residualMap.get(LogisticRegressionResult.SORTED_RESIDUALS);
		int[] sortedResidualsIndex = 
			(int[])residualMap.get(LogisticRegressionResult.SORTED_RESIDUALS_INDEX);
		double[] sortedNormalQuantiles = 
			(double[])residualMap.get(LogisticRegressionResult.SORTED_NORMAL_QUANTILES);

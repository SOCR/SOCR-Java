...
public class OneTExamples extends ExampleData {
	...
	public  OneTExamples(int analysisType, int exampleID) {


		switch (exampleID) {
			case ExampleData.ONE_T_1: { // and for ONE_T.
			...
			case ExampleData.ONE_T_2:  { // and for ONE_T.
				// rice, page 433
				...
				for (int i = 0; i < sampleSize; i++) {
					example[i][0] = AnalysisUtility.truncateDigits((test[i] - control[i]), 4) + "";
				}
				dataTable = new JTable(example,  columnNames);
			...
			case ExampleData.ONE_T_3 : {
				...
				for (int i = 0; i < sampleSize; i++) {
					example[i][0] = AnalysisUtility.truncateDigits((a[i] - b[i]), 4) + "";
				}
				dataTable = new JTable(example,  columnNames);

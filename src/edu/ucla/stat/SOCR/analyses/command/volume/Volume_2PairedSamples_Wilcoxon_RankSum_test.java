...
/*
 * This class provides an image-volume based 2-paired-samples Wilcoxon Rank-Sum test analysis for 1D, 2D and 3D imaging volumes, based
 ...
 * 			edu.ucla.stat.SOCR.analyses.command.volume.Volume_2PairedSamples_Wilcoxon_RankSum_test 
 * 			-dm DesignMatrix.txt -h -mask [Mask-volume.img] -regressors [name1=Label1,Label2]
 ...
 * 			-regressors [name1=Label1,Label2]: specify which column/variable should be used and what are
 * 									the values identifying the 2 (paired) samples!
 ...
 * 			-t [wStat_Filename]:	output the W-Statistics for the group differences
 ...
 * 
 * Documentation: 
 * 	http://wiki.stat.ucla.edu/socr/index.php/SOCR_EduMaterials_AnalysesCommandLineVolume2PairedSamples_Wilcoxon_RankSum
 * 
 */

public class Volume_2PairedSamples_Wilcoxon_RankSum_test {
	...
	static FileOutputStream pFOS, wFOS;
	static BufferedOutputStream pBOS, wBOS;
	static MemoryCacheImageOutputStream pMciis, wMciis;
	...
	static boolean outputWilcoxonStatVolumes=false;
	...
	public static void main(String[] args) {
		...
		System.out.println(
		"Docs: http://wiki.stat.ucla.edu/socr/index.php/SOCR_EduMaterials_AnalysesCommandLineVolume2Samples_WilcoxonT_test");
		...
		try {
			for (i=0; i<args.length; i++) {
				if (args[i].compareToIgnoreCase("-h")==0) header=true;
					...
					outputWilcoxonStatVolumes=true;
					wStat_Filename = args[++i];
					System.out.println("wStat_Filename="+wStat_Filename);
					...
					"Docs: http://wiki.stat.ucla.edu/socr/index.php/SOCR_EduMaterials_AnalysesCommandLineVolume2PairedSamples_Wilcoxon_RankSum");
					return;
		...
		try {
			...
			while ( (line = bReader.readLine()) != null) {
					...
					if (varHeader[0]==null && header) {		// Read Header Row
						...
						System.out.println("No Independent Variable Specified (no header row) 1. Cannot run the regression\n"+
						"Docs: http://wiki.stat.ucla.edu/socr/index.php/SOCR_EduMaterials_AnalysesCommandLineVolume2PairedSamples_Wilcoxon_RankSum");
						return;
						...
						try {
							...
							//e.printStackTrace();
							System.out.println("Volume 2-Sample Wilcoxon Rank-Sum test: 1");
							return;
						} catch (Exception e) {
							//e.printStackTrace();
							System.out.println("Volume 2-Sample Wilcoxon Rank-Sum test: 2");
							return;
						}
					}
			}
		...
		if (!header)
		{	System.out.println("No Independent Variable Specified in Design-Matrix. (2) Cannot run the analysis\n"+
			"Docs: http://wiki.stat.ucla.edu/socr/index.php/SOCR_EduMaterials_AnalysesCommandLineVolume2PairedSamples_Wilcoxon_RankSum");
			return;
		}
		...
		System.out.println("Paired Group Sizes Must be Equal: Group_1("+regressorLabels[0]+")= "+ xLengthGrp1+
		...
		try { 
			...
			if (outputWilcoxonStatVolumes) {
					wFOS = new FileOutputStream(wStat_Filename+
						"_wStat_"+regressors+"_"+regressorLabels[0]+"_"+regressorLabels[1]+".img");
					wBOS = new BufferedOutputStream(wFOS, bufferSize);
					wMciis = new MemoryCacheImageOutputStream(wBOS);
					setByteOrder(wMciis, byteorder);
			}
				...
				for(j = 0; j < dim[1]; j++) {
					for(k = 0; k < dim[0]; k++) {
				        ...
				        if (maskInputVolume==null || (maskInputVolume!=null && maskVolumeBoolean[i][j][k]==true)) {
							...
							diffMeasure = 0;
							for (int index=0; index<yDataGrp1.length; index++)
								diffMeasure += Math.abs(yDataGrp1[index]-yDataGrp2[index]);
							
							if (diffMeasure>0) {	// Check to make sure we need to compare the 2 groups (only of they are distinct!)
								...
								result = null;
								try {
									result = (TwoPairedSignedRankResult)data.getAnalysis(AnalysisType.TWO_PAIRED_SIGNED_RANK);
								} catch (Exception e) { System.out.println(e); }

								wStat = (float)result.getZScore(); 
								pValue = (float)result.getPValueTwoSided();
							} else {
								wStat = 0; 
								pValue = 1;
							}
							
							putNextOutputDataBuffer(wStat, pValue);
			...
			if (outputWilcoxonStatVolumes) {
				...
				wFOS.flush();
				wFOS.close();
				System.out.println("Wilcoxon-Stats Output File ="+wStat_Filename+
					"_wStat_"+regressors+"_"+regressorLabels[0]+"_"+regressorLabels[1]+".img");
			}
			...
			System.err.println("Volume2PairedSamples_Wilcoxon_SignedRank_test Error!!!!!!!!!!!!!!");
		}
	 ...
	 * @param wStat t-statistics
	 * @param pValue - p-value
	 */
	public static void putNextOutputDataBuffer(float wStat, float pValue) {
		...
		if (outputWilcoxonStatVolumes) {
			try {
				//wMciis.writeFloat(wStat);
				if (byteswap==false) wMciis.writeFloat(wStat);
				else wMciis.writeFloat(swap(wStat));
			} catch (IOException e) { System.err.println(e);  }
		}
	...
	public static void putNextOutputDataBuffer() {
    	...
    	if (outputWilcoxonStatVolumes==true) {
    		try {
    			if (byteswap==false) wMciis.writeFloat(tempFloat);
    			else wMciis.writeFloat(swap(tempFloat));
    		} catch (IOException e) { System.err.println(e); }
    	}
	} 

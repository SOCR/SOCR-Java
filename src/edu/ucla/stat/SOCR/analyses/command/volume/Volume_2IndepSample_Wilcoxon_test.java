...
/*
 * This class provides an image-volume based 2-independent-samples Wilcoxon-test analysis for 1D, 2D and 3D imaging volumes, based
 ...
 * 			edu.ucla.stat.SOCR.analyses.command.volume.Volume_2IndepSample_Wilcoxon_test 
 ...
 * 									the values identifying the 2 samples!
 ...
 * 			-t [wStat_Filename]:	output the T-Statistics for the effect-size Beta
 ...
 * 
 * Documentation: 
 * 	http://wiki.stat.ucla.edu/socr/index.php/SOCR_EduMaterials_AnalysesCommandLineVolume2Samples_Wilcoxon_test
 * 
 */

public class Volume_2IndepSample_Wilcoxon_test {
	...
	public static void main(String[] args) {
		...
		System.out.println(
		"Docs: http://wiki.stat.ucla.edu/socr/index.php/SOCR_EduMaterials_AnalysesCommandLineVolume2IndepSample_Wilcoxon_test");
		...
		try {
			for (i=0; i<args.length; i++) {
				if (args[i].compareToIgnoreCase("-h")==0) header=true;
					...
					"Docs: http://wiki.stat.ucla.edu/socr/index.php/SOCR_EduMaterials_AnalysesCommandLineVolume2Samples_Wilcoxon_test");
					return;
		...
		System.out.println("Sizes: Group_1("+regressorLabels[0]+")= "+ xLengthGrp1+
		...
		try { 
				...
				for(j = 0; j < dim[1]; j++) {
					for(k = 0; k < dim[0]; k++) {
				        ...
				        if (maskInputVolume==null || (maskInputVolume!=null && maskVolumeBoolean[i][j][k]==true)) {
							...
							if (TwoIndependentWilcoxonResult.IS_LARGE_SAMPLE.compareToIgnoreCase("true")==0) {
								// for large sample-sizes use Normal Approximation (n1 and n2 >=10)
								wStat = (float)result.getZScore(); 
								pValue = (float)result.getPValueTwoSided();
							} else {	// For smaller sample-sizes use Exact Wilxocon test
								wStat = (float)result.getUStatSmallerGroup(); 
								pValue = (float)result.getPValueTwoSided();
							}
			...
			System.err.println("Volume2Sample_Wilcoxon_test Error!!!!!!!!!!!!!!\n Exception: "+e);
		}

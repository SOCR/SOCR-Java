...
public class TwoIndependentTUnpooled extends Analysis implements PropertyChangeListener {
	...
	public void doAnalysis(){
		...
		******************************************************************/
		...
		try {
			tStatUnpooled = result.getTStatUnpooled();//((Double)result.getTexture().get(TwoIndependentTResult.T_STAT)).doubleValue();
		} catch (Exception e) {
			//showError("Excepion3 " +e + "");
		}
		try {
			pValueOneSidedUnpooled = result.getPValueOneSidedUnpooled();//((String)result.getTexture().get(TwoIndependentTResult.P_VALUE));
		} catch (Exception e) {
			//showError(e + "");
		}
		try {
			pValueTwoSidedUnpooled = result.getPValueTwoSidedUnpooled();//((String)result.getTexture().get(TwoIndependentTResult.P_VALUE));
		} catch (Exception e) {
			//showError(e + "");
		}
	...
	resultPanelTextArea.append("\n\tT-Statistics (Unpooled) = " + result.getFormattedDouble(tStatUnpooled));
	resultPanelTextArea.append("\n\tOne-Sided P-Value (Unpooled) = " + result.getFormattedDouble(pValueOneSidedUnpooled));
	resultPanelTextArea.append("\n\tTwo-Sided P-Value (Unpooled) = " + result.getFormattedDouble(pValueTwoSidedUnpooled));

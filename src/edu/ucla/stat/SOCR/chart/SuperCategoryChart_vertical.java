 ...
 */
public class SuperCategoryChart_vertical extends Chart implements PropertyChangeListener {
	 protected CategoryDataset dataset;
	 
	public void init(){

		depLabel = new JLabel("Label");
		indLabel = new JLabel("Cases");
    ...
    protected  CategoryDataset createDataset(boolean isDemo) {
		...
		if (isDemo){
			...
			int[][] skipx = new int[xyLength][independentVarLength];
			...
			try{
				...
				for (int j=0; j<xyLength; j++)
					for (int i=0; i<independentVarLength; i++){
						if(y[j][0]!=null && skipx[j][i]!=1)
							((DefaultCategoryDataset) dataset).addValue(x[j][i], independentHeaders[i], y[j][0]); 
						//System.out.println("adding :("+x[j][i]+","+independentHeaders[i]+","+y[j][0]+")");
				}
  ...
  public void resetExample() {
        ...
        hasExample = true;
		convertor.dataset2Table_vertical(dataset);				

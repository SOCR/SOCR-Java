    ...
    protected  XYDataset createDataset(boolean isDemo) {
		if (isDemo)	{
			...
			for (int j =0; j<independentVarLength; j++)
				rangeLabel+=independentHeaders[j]+"/";
			...
			for (int j=0; j<independentVarLength; j++){
				...
				for (int i=0; i<row_count;i++){
					if(skip[j][i]==false)
						series.add(y_freq[j][i], raw_xvalue[j][i]);
					//	System.out.println("updating "+raw_xvalue[j][i]+" f="+y_freq[j][i]);

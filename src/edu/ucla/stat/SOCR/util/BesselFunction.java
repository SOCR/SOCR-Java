 ...
 */
public class BesselFunction extends BesselConstants {
	/****************************************
...
static public double i0(double x) throws ArithmeticException {
	...
	if( x <= 8.0 ) {
		y = (x/2.0) - 2.0;
		return( Math.exp(x) * BesselArithmetic.chbevl( y, A_i0, 30 ) );
	}

	return(  Math.exp(x) * BesselArithmetic.chbevl( 32.0/x - 2.0, B_i0, 25 ) / Math.sqrt(x) );
}
/**
...
static public double i0e(double x) throws ArithmeticException {
	...
	if( x <= 8.0 ) {
		y = (x/2.0) - 2.0;
		return( BesselArithmetic.chbevl( y, A_i0, 30 ) );
	}

	return(  BesselArithmetic.chbevl( 32.0/x - 2.0, B_i0, 25 ) / Math.sqrt(x) );
}
/**
...
static public double i1(double x) throws ArithmeticException {
	...
	if( z <= 8.0 )
		{
		y = (z/2.0) - 2.0;
		z = BesselArithmetic.chbevl( y, A_i1, 29 ) * z * Math.exp(z);
		}
	else
		{
		z = Math.exp(z) * BesselArithmetic.chbevl( 32.0/z - 2.0, B_i1, 25 ) / Math.sqrt(z);
		}
	if( x < 0.0 )
		z = -z;
	return( z );
...
if( z <= 8.0 )
	{
	y = (z/2.0) - 2.0;
	z = BesselArithmetic.chbevl( y, A_i1, 29 ) * z;
	}
else
	{
	z = BesselArithmetic.chbevl( 32.0/z - 2.0, B_i1, 25 ) / Math.sqrt(z);
	}
if( x < 0.0 )
	z = -z;
return( z );
...
/**
 * Returns the BesselFunction function of the first kind of order <tt>n</tt> of the argument.
 * @param n the order of the BesselFunction function.
 * @param x the value to compute the bessel function of.
 */
...
/**
 * Returns the modified BesselFunction function of the third kind
 * of order 0 of the argument.
 * <p>
 * The range is partitioned into the two intervals [0,8] and
 * (8, infinity).  Chebyshev polynomial expansions are employed
...
static public double k0(double x) throws ArithmeticException {
	...
	if( x <= 2.0 ) {
		y = x * x - 2.0;
		y = BesselArithmetic.chbevl( y, A_k0, 10 ) - Math.log( 0.5 * x ) * i0(x);
		return( y );
	}
	
	z = 8.0/x - 2.0;
	y = Math.exp(-x) * BesselArithmetic.chbevl( z, B_k0, 25 ) / Math.sqrt(x);
	return(y);
}
/**
 * Returns the exponentially scaled modified BesselFunction function
...
static public double k0e(double x) throws ArithmeticException {
	...
	y = BesselArithmetic.chbevl( 8.0/x - 2.0, B_k0, 25 ) / Math.sqrt(x);
	return(y);
}
/**
 * Returns the modified BesselFunction function of the third kind
...
static public double k1(double x) throws ArithmeticException {
	...
	if( x <= 2.0 ) {
		y = x * x - 2.0;
		y =  Math.log(z) * i1(x)  +  BesselArithmetic.chbevl( y, A_k1, 11 ) / x;
		return( y );
	}

	return(  Math.exp(-x) * BesselArithmetic.chbevl( 8.0/x - 2.0, B_k1, 25 ) / Math.sqrt(x) );
}
/**
 * Returns the exponentially scaled modified BesselFunction function
...
static public double k1e(double x) throws ArithmeticException {
	...
	if( x <= 2.0 ) {
		y = x * x - 2.0;
		y =  Math.log( 0.5 * x ) * i1(x)  +  BesselArithmetic.chbevl( y, A_k1, 11 ) / x;
		return( y * Math.exp(x) );
	}

	return(  BesselArithmetic.chbevl( 8.0/x - 2.0, B_k1, 25 ) / Math.sqrt(x) );
}

/**
 * Returns the modified BesselFunction function of the first kind
 * of order <tt>nn</tt> of the argument.
 * <p>
 * This is an approximate solution using the formula here:
 ...
 *
 * @param nn the order of the BesselFunction function.
 * @param x the value to compute the bessel function of.
 */
static public double in(int alpha, double y) throws ArithmeticException {
	if( y <= 0.0 ) throw new IllegalArgumentException();
	double finiteSeriesApprox = 0;
    for (int j=0; j<100; j++) {
    	finiteSeriesApprox += Math.pow(y*y/4, j)/(BesselArithmetic.factorial(j)*
    			edu.ucla.stat.SOCR.core.Distribution.gamma(alpha+j+1)); 
    }
    return Math.pow(y/2, alpha)*finiteSeriesApprox;
}
...
/**
 * Returns the BesselFunction function of the second kind of order <tt>n</tt> of the argument.
 * @param n the order of the BesselFunction function.
 * @param x the value to compute the bessel function of.
 */

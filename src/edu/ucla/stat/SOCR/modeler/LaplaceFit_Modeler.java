...
public class LaplaceFit_Modeler extends Modeler implements ItemListener{
	
	public edu.ucla.stat.SOCR.distributions.LaplaceDistribution LaplaceDistr;	
	...
	//default constructor
	public LaplaceFit_Modeler(){
	}
	
	//constructor
	public LaplaceFit_Modeler(JPanel controlpanel) {
	...
	public void fitCurve(float[] rawDat, float minx, float maxx,
		...
		try{
			if(estimateParams.isSelected()){
				...
				//LaplaceDistr = new edu.ucla.stat.SOCR.distributions.LaplaceDistribution(rawDat_d);
				LaplaceDistr.paramEstimate(rawDat_d);
				
			}
			else
				LaplaceDistr = new edu.ucla.stat.SOCR.distributions.LaplaceDistribution(Double.parseDouble(locationParamField.getText()),Double.parseDouble(scaleParamField.getText()));

			resultPanelTextArea.setText("Location Parameter = "+ Double.toString(LaplaceDistr.getMu())+"\nScale Parameter = " + Double.toString(LaplaceDistr.getBeta())+"\n\n");
			...
			for(int i = 0; i<number ;i++) {
				modelX[i] = minx+ind*i;
				modelY[i] = LaplaceDistr.getDensity(modelX[i]);
			}
			...
			double[] y = new double[numberOfQuantiles];
				//	 Model Quantiles
			for (int i = 0; i < numberOfQuantiles; i++)  y[i] = LaplaceDistr.getQuantile((i+0.5)/numberOfQuantiles);
				//	 Data Quantiles
			for (int i= 0; i<rawDat.length; i++) temp[i] = (double)rawDat[i];
			x = edu.ucla.stat.SOCR.util.AnalysisUtility.getQuantileArray(temp);
			resultPanelTextArea.append(getKSModelTestString(LaplaceDistr.getName(), numberOfQuantiles, x, y));
	...
	public double[] generateSamples(int sampleCount) {
		...
		for(int i = 0; i< sampleCount;i++)
			dat[i] = LaplaceDistr.simulate();
		return dat;
	}
	
	public String getDescription() {
		String desc = new String();
		desc = "URL: http://socr.ucla.edu/htmls/dist/Laplace_Distribution.html";
		return desc;
	}
	...
	public boolean isContinuous() {
		return LaplaceFit_Modeler.isContinuous;
	}

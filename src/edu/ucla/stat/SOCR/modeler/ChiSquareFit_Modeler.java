...
/**This class Designs a ChiSquare Fit model fitting curve. */
public class ChiSquareFit_Modeler extends Modeler implements ItemListener {
	...
	public edu.ucla.stat.SOCR.distributions.ChiSquareDistribution ChiSquareDistr;
	public float kernalVar = 0;
	private double minX = 0;
	private double maxX = 0;
	private int modelType = CONTINUOUS_DISTRIBUTION_TYPE;
	public JTextField dfParamField = new JTextField("1",3);
	public JLabel dfParamLabel = new JLabel("Degrees of Freedom");
	...
	public static int SLICE_SIZE = 8001;
      ...
      */
    public ChiSquareFit_Modeler() {
    }

    /** 
     * ChiSquareFit_Modeler Constructor.
     */
    public ChiSquareFit_Modeler(JPanel controlpanel) {
    ...
    public void registerObservers(ObservableWrapper o) {
	    ...
	    o.addJTextField(dfParamField);
    }

    public void addParams(JPanel controlpanel) {
        controlpanel.add(dfParamLabel);
        controlpanel.add(dfParamField);
    }
	...
	public double[] returnModelY() {
     ...
     public void toggleParams(boolean istrue) {
        if(istrue) {
            dfParamLabel.setVisible(true);
            dfParamField.setVisible(true);
        } else {
            dfParamLabel.setVisible(false);
            dfParamField.setVisible(false);
        }
    }
	...
	public double[] generateSamples( int sampleCount) {
		...
		ChiSquareDistr = new edu.ucla.stat.SOCR.distributions.ChiSquareDistribution(
				(int)(Double.parseDouble(dfParamField.getText())));
		for(int i = 0; i< sampleCount;i++)
			dat[i] = ChiSquareDistr.simulate();
		return dat;
	}
	public int getModelCount() {
		return modelCt;
	}
	public void fitCurve(float[] rawDat, double minx, double maxx,JTextArea resultPanelTextArea, 
			boolean rescaleClicked, boolean scaleUp, boolean initReset) {
	...
	public void fitCurve(float[] rawDat, float minx, float maxx,
		...
		try{
			...
			if(estimateParams.isSelected())
				ChiSquareDistr = new edu.ucla.stat.SOCR.distributions.ChiSquareDistribution(rawDat);
			else
				ChiSquareDistr = new edu.ucla.stat.SOCR.distributions.ChiSquareDistribution(
						(int)(Double.parseDouble(dfParamField.getText())));
			
			double _df = ChiSquareDistr.getDegrees();
			String dfString = Double.toString(_df);

			resultPanelTextArea.setText("df = "+ dfString+"\n\n");
			float ind = 1;
			...
			for(int i = 0; i<number ;i++) {
				modelX[i] = minx+ind*i;
				modelY[i] = ChiSquareDistr.getDensity(modelX[i]);
			}
			...
			double[] y = new double[numberOfQuantiles];
				//	 Model Quantiles
			for (int i = 0; i < numberOfQuantiles; i++)  y[i] = ChiSquareDistr.getQuantile((i+0.5)/numberOfQuantiles);
				//	 Data Quantiles
			for (int i= 0; i<rawDat.length; i++) temp[i] = (double)rawDat[i];
			x = edu.ucla.stat.SOCR.util.AnalysisUtility.getQuantileArray(temp);
			resultPanelTextArea.append(getKSModelTestString(ChiSquareDistr.getName(), numberOfQuantiles, x, y));
	...
	public void fitCurve(float[] rawDat, float minx, float maxx, double mu0, double sigma, boolean plotBlocks, boolean scaleUp) {
		...
		try{
			int count = 0;
			plotBlocks = false;
			if(plotBlocks) ChiSquareDistr = 
				new edu.ucla.stat.SOCR.distributions.ChiSquareDistribution(rawDat);
			else ChiSquareDistr = 
				new edu.ucla.stat.SOCR.distributions.ChiSquareDistribution((int)(mu0));

			// String meanString = Double.toString(ChiSquareDistr.getMean());
			// String varString = Double.toString(ChiSquareDistr.getVariance());
	...
	public String getDescription() {
		if (ChiSquareDistr==null) ChiSquareDistr = 
			new edu.ucla.stat.SOCR.distributions.ChiSquareDistribution(
					(int)(Double.parseDouble(dfParamField.getText())));
		return ChiSquareDistr.getOnlineDescription();
	}
	
    /** Returns the instructions for using this modeler*/
    public String getInstructions() {
    	    String instructions = new String();
    	    instructions = "http://wiki.stat.ucla.edu/socr/index.php/Help_pages_for_SOCR_Modeler";
    	    return instructions;
    }
    ...
    public String getResearch() {
    	    ...
    	    research = "http://en.wikipedia.org/wiki/Chi_square_distribution";
    	    return research;
    }

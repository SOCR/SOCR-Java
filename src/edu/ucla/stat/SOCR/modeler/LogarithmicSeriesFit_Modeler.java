...
public class LogarithmicSeriesFit_Modeler extends Modeler implements ItemListener{
	
	public edu.ucla.stat.SOCR.distributions.LogarithmicSeriesDistribution LogSeriesDistr;	
	...
	public JTextField ParamField = new JTextField("0.5",3);
	
	private ButtonGroup buttonGroup = new ButtonGroup();
	

	//default constructor
	public LogarithmicSeriesFit_Modeler(){
	}
	
	//constructor
	public LogarithmicSeriesFit_Modeler(JPanel controlpanel) {
	...
	public void fitCurve(float[] rawDat, float minx, float maxx,
		...
		try{
			if(estimateParams.isSelected())
				LogSeriesDistr = new edu.ucla.stat.SOCR.distributions.LogarithmicSeriesDistribution(rawDat);
			else
				LogSeriesDistr = new edu.ucla.stat.SOCR.distributions.LogarithmicSeriesDistribution(Double.parseDouble(ParamField.getText()));

			resultPanelTextArea.setText("Shape = "+ Double.toString(LogSeriesDistr.getShape())+"\n\n");
			...
			for(int i = 0; i<number ;i++) {
				modelX[i] = minx+ind*i;
				modelY[i] = LogSeriesDistr.getDensity(modelX[i]);
			}
			...
			double[] y = new double[numberOfQuantiles];
				//	 Model Quantiles
			for (int i = 0; i < numberOfQuantiles; i++)  y[i] = LogSeriesDistr.getQuantile((i+0.5)/numberOfQuantiles);
				//	 Data Quantiles
			for (int i= 0; i<rawDat.length; i++) temp[i] = (double)rawDat[i];
			x = edu.ucla.stat.SOCR.util.AnalysisUtility.getQuantileArray(temp);
			resultPanelTextArea.append(getKSModelTestString(LogSeriesDistr.getName(), numberOfQuantiles, x, y));
	...
	public double[] generateSamples(int sampleCount) {
		...
		for(int i = 0; i< sampleCount;i++)
			dat[i] = LogSeriesDistr.simulate();
		return dat;
	}
	
	public String getDescription() {
		String desc = new String();
		desc = "URL: http://socr.ucla.edu/htmls/dist/LogarithmicSeries_Distribution.html";
		return desc;
	}
	...
	public boolean isContinuous() {
		return LogarithmicSeriesFit_Modeler.isContinuous;
	}

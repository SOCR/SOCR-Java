...
public class LogisticFit_Modeler extends Modeler implements ItemListener{
	
	public edu.ucla.stat.SOCR.distributions.LogisticDistribution LogisticDistr;	
	...
	//default constructor
	public LogisticFit_Modeler(){
	}
	
	//constructor
	public LogisticFit_Modeler(JPanel controlpanel) {
	...
	public void fitCurve(float[] rawDat, float minx, float maxx,
		...
		try{
			if(estimateParams.isSelected())
				LogisticDistr = new edu.ucla.stat.SOCR.distributions.LogisticDistribution(rawDat);
			else
				LogisticDistr = new edu.ucla.stat.SOCR.distributions.LogisticDistribution(Double.parseDouble(locationParamField.getText()),Double.parseDouble(scaleParamField.getText()));

			
//is this correct?
//LogisticDistribution(mu, sigma)
//where sigma = SD = scale
			resultPanelTextArea.setText("Location Parameter = "+ Double.toString(LogisticDistr.getMean())+"\nScale Parameter = " + Double.toString(LogisticDistr.getSD())+"\n\n");
			...
			for(int i = 0; i<number ;i++) {
				modelX[i] = minx+ind*i;
				modelY[i] = LogisticDistr.getDensity(modelX[i]);
			}
			...
			double[] y = new double[numberOfQuantiles];
				//	 Model Quantiles
			for (int i = 0; i < numberOfQuantiles; i++)  y[i] = LogisticDistr.getQuantile((i+0.5)/numberOfQuantiles);
				//	 Data Quantiles
			for (int i= 0; i<rawDat.length; i++) temp[i] = (double)rawDat[i];
			x = edu.ucla.stat.SOCR.util.AnalysisUtility.getQuantileArray(temp);
			resultPanelTextArea.append(getKSModelTestString(LogisticDistr.getName(), numberOfQuantiles, x, y));
	...
	public double[] generateSamples(int sampleCount) {
		...
		for(int i = 0; i< sampleCount;i++)
			dat[i] = LogisticDistr.simulate();
		return dat;
	}
	
	public String getDescription() {
		String desc = new String();
		desc = "URL: http://socr.ucla.edu/htmls/dist/Logistic_Distribution.html";
		return desc;
	}
	...
	public boolean isContinuous() {
		return LogisticFit_Modeler.isContinuous;
	}

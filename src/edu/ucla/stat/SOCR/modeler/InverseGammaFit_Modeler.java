...
public class InverseGammaFit_Modeler extends Modeler implements ItemListener{
	
	public edu.ucla.stat.SOCR.distributions.InverseGammaDistribution InvGammaDistr;	
	...
	//default constructor
	public InverseGammaFit_Modeler(){
	}
	
	//constructor
	public InverseGammaFit_Modeler(JPanel controlpanel) {
	...
	public void fitCurve(float[] rawDat, float minx, float maxx,
		...
		try{
			if(estimateParams.isSelected())
				InvGammaDistr = new edu.ucla.stat.SOCR.distributions.InverseGammaDistribution(rawDat);
			else
				InvGammaDistr = new edu.ucla.stat.SOCR.distributions.InverseGammaDistribution(Double.parseDouble(shapeParamField.getText()),Double.parseDouble(scaleParamField.getText()));

			resultPanelTextArea.setText("Shape Parameter = "+ Double.toString(InvGammaDistr.getShape())+"\nScale Parameter = " + Double.toString(InvGammaDistr.getScale())+"\n\n");
			...
			for(int i = 0; i<number ;i++) {
				modelX[i] = minx+ind*i;
				modelY[i] = InvGammaDistr.getDensity(modelX[i]);
			}
			...
			double[] y = new double[numberOfQuantiles];
				//	 Model Quantiles
			for (int i = 0; i < numberOfQuantiles; i++)  y[i] = InvGammaDistr.getQuantile((i+0.5)/numberOfQuantiles);
				//	 Data Quantiles
			for (int i= 0; i<rawDat.length; i++) temp[i] = (double)rawDat[i];
			x = edu.ucla.stat.SOCR.util.AnalysisUtility.getQuantileArray(temp);
			resultPanelTextArea.append(getKSModelTestString(InvGammaDistr.getName(), numberOfQuantiles, x, y));
	...
	public double[] generateSamples(int sampleCount) {
		...
		for(int i = 0; i< sampleCount;i++)
			dat[i] = InvGammaDistr.simulate();
		return dat;
	}
	
	public String getDescription() {
		String desc = new String();
		desc = "URL: http://socr.ucla.edu/htmls/dist/InverseGamma_Distribution.html";
		return desc;
	}
	...
	public boolean isContinuous() {
		return InverseGammaFit_Modeler.isContinuous;
	}

...
public class ChiFit_Modeler extends Modeler implements ItemListener{
	
	public edu.ucla.stat.SOCR.distributions.ChiDistribution ChiDistr;	
	...
	public JLabel freedomParamLabel = new JLabel("Degree of Freedom");
	public JTextField freedomParamField = new JTextField("1",3);
	
	private ButtonGroup buttonGroup = new ButtonGroup();
	

	//default constructor
	public ChiFit_Modeler(){
	}
	
	//constructor
	public ChiFit_Modeler(JPanel controlpanel) {
	...
	public void addParams(JPanel controlpanel){
		controlpanel.add(freedomParamLabel);
		controlpanel.add(freedomParamField);
	}
	...
	public void fitCurve(float[] rawDat, float minx, float maxx,
		...
		try{
			if(estimateParams.isSelected())
				ChiDistr = new edu.ucla.stat.SOCR.distributions.ChiDistribution(rawDat);
			else
				ChiDistr = new edu.ucla.stat.SOCR.distributions.ChiDistribution(Integer.parseInt(freedomParamField.getText()));

			resultPanelTextArea.setText("Degrees of Freedom = "+ Double.toString(ChiDistr.getDegrees())+"\n\n");
			...
			for(int i = 0; i<number ;i++) {
				modelX[i] = minx+ind*i;
				modelY[i] = ChiDistr.getDensity(modelX[i]);
			}
			...
			double[] y = new double[numberOfQuantiles];
				//	 Model Quantiles
			for (int i = 0; i < numberOfQuantiles; i++)  y[i] = ChiDistr.getQuantile((i+0.5)/numberOfQuantiles);
				//	 Data Quantiles
			for (int i= 0; i<rawDat.length; i++) temp[i] = (double)rawDat[i];
			x = edu.ucla.stat.SOCR.util.AnalysisUtility.getQuantileArray(temp);
			resultPanelTextArea.append(getKSModelTestString(ChiDistr.getName(), numberOfQuantiles, x, y));
	...
	public double[] generateSamples(int sampleCount) {
		...
		for(int i = 0; i< sampleCount;i++)
			dat[i] = ChiDistr.simulate();
		return dat;
	}
	
	public String getDescription() {
		String desc = new String();
		desc = "URL: http://socr.ucla.edu/htmls/dist/Chi_Distribution.html";
		return desc;
	}
	...
	public boolean isContinuous() {
		return ChiFit_Modeler.isContinuous;
	}
    ...
    public void registerObservers(ObservableWrapper o) {
		...
		o.addJTextField(freedomParamField);
    }
    ...
    public void toggleParams(boolean istrue) {
        if(istrue) {
            freedomParamLabel.setVisible(true);
            freedomParamField.setVisible(true);

        }else {
            freedomParamLabel.setVisible(false);
            freedomParamField.setVisible(false);
        }
    }	

...
public class ParetoFit_Modeler extends Modeler implements ItemListener{
	
	public edu.ucla.stat.SOCR.distributions.ParetoDistribution ParetoDistr;	
	...
	public JLabel alphaParams = new JLabel("alpha (power) Parameter");
	public JTextField alphaParamField = new JTextField("1",3);
	public JLabel thetaParams = new JLabel("theta (left start) Parameter");
	public JTextField thetaParamField = new JTextField("1",3);
	...
	//default constructor
	public ParetoFit_Modeler(){
	}
	
	//constructor
	public ParetoFit_Modeler(JPanel controlpanel) {
	...
	public void addParams(JPanel controlpanel){
		controlpanel.add(alphaParams);
		controlpanel.add(alphaParamField);
		controlpanel.add(thetaParams);
		controlpanel.add(thetaParamField);
	}
	...
	public void fitCurve(float[] rawDat, float minx, float maxx,
		...
		try{
			if(estimateParams.isSelected()){
				ParetoDistr = new edu.ucla.stat.SOCR.distributions.ParetoDistribution();
				...
				ParetoDistr.paramEstimate(rawDat_d);
				
			}	
			else
				ParetoDistr = new edu.ucla.stat.SOCR.distributions.ParetoDistribution(Integer.parseInt(alphaParamField.getText()), Integer.parseInt(thetaParamField.getText()));

			resultPanelTextArea.setText("alpha (power) Parameter = "+ Double.toString(ParetoDistr.getParameter1())+"\ntheta (left start) Parameter = " + Double.toString(ParetoDistr.getParameter2())+"\n\n");
			...
			for(int i = 0; i<number ;i++) {
				modelX[i] = minx+ind*i;
				modelY[i] = ParetoDistr.getDensity(modelX[i]);
			}
			...
			double[] y = new double[numberOfQuantiles];
				//	 Model Quantiles
			for (int i = 0; i < numberOfQuantiles; i++)  y[i] = ParetoDistr.getQuantile((i+0.5)/numberOfQuantiles);
				//	 Data Quantiles
			for (int i= 0; i<rawDat.length; i++) temp[i] = (double)rawDat[i];
			x = edu.ucla.stat.SOCR.util.AnalysisUtility.getQuantileArray(temp);
			resultPanelTextArea.append(getKSModelTestString(ParetoDistr.getName(), numberOfQuantiles, x, y));
	...
	public double[] generateSamples(int sampleCount) {
		...
		for(int i = 0; i< sampleCount;i++)
			dat[i] = ParetoDistr.simulate();
		return dat;
	}
	
	public String getDescription() {
		String desc = new String();
		desc = "URL: http://socr.ucla.edu/htmls/dist/Pareto_Distribution.html";
		return desc;
	}
	...
	public boolean isContinuous() {
		return ParetoFit_Modeler.isContinuous;
	}
    ...
    public void registerObservers(ObservableWrapper o) {
		...
		o.addJTextField(alphaParamField);
		o.addJTextField(thetaParamField);
    }
    ...
    public void toggleParams(boolean istrue) {
        if(istrue) {
            alphaParams.setVisible(true);
            alphaParamField.setVisible(true);
            thetaParams.setVisible(true);
            thetaParamField.setVisible(true);

        }else {
            alphaParams.setVisible(false);
            alphaParamField.setVisible(false);
            thetaParams.setVisible(false);
            thetaParamField.setVisible(false);
        }
    }	

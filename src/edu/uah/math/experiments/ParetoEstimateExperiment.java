...
/**
* This class models the point estimation problem of the Pareto distribution. A random
* sample from the Pareto distribution, with shape paramater a is simulated. Standard
* point estimates of a are computed, along with the empirical
...
public class ParetoEstimateExperiment extends EstimateExperiment implements Serializable{
	...
	private Parameter aScroll = new Parameter(1, 25, 0.5, a, "Shape Parameter", "a");
	private ParetoDistribution dist = new ParetoDistribution(a);
	...
	public void init(){
		...
		//U Table
		uTable.setDescription("Bias and mean squre error of U as an estimator of a");
		uTable.setShow(StatisticsTable.DATA);
		addComponent(uTable, 1, 1, 1, 1);
		//V Table
		vTable.setDescription("Bias and mean squre error of V as an estimator of a");
	...
	*/
	...
	/**
	* This method defines the experiment.  A sample of size n from the Pareto distribuion
	...
	*/
	...
	/**
	* This method handles the scroll events for changing the parameter. The
